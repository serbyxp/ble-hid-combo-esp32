    ...
    if (event_base == WIFI_EVENT){
        switch(event_id){
            ...
            case WIFI_EVENT_AP_STOP:
                        ...
                        s_ap_running = false;
                        // Stop HTTP server when AP goes down
...
void wifi_ap_stop(){
    // Stop HTTP server for the AP interface, but avoid stopping the
    // entire Wi-Fi stack if a STA connection is active. If we're in
    // APSTA mode, switch to STA-only. If we were AP-only, stop Wi-Fi.
    extern void http_server_stop();
    http_server_stop();
    vTaskDelay(pdMS_TO_TICKS(100));
